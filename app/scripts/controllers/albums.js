...
var albumsCtrl = ['$scope', '$route', '$routeParams', 'gtlFind', 'Albums', '$filter', function ($scope, $route, $routeParams, gtlFind, Albums, $filter ) {
    ...
    if (typeof($routeParams.id) !== 'undefined' && $filter('isNumeric')($routeParams.id)) {
        ...
        $scope.albums = $filter('filter')($scope.albums, {id:index});
    }
}];
